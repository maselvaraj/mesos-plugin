...
/**
 * Allows to access a workspace through jenkins master.
 * Primary use case is cloud based CI implementation where slaves
 * won't be there online forever. Note that control comes here only
 * when nodes are not accessible or offline. Make sure that workspace are
 * created in mounts/filer which is accessible from jenkins master. 
 */
@Extension
			...
			if(mesosCloud != null) {
				...
				for (MesosSlaveInfo mesosSlaveInfo : slaveInfos) {
					if(mesosSlaveInfo.getLabelString().equals(assignedLabel)) {
						String workspacePath = mesosSlaveInfo.getCustomRemoteFSRoot()+File.separator+WORKSPACE+File.separator+jobName;
