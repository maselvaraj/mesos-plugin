...
public class MesosItemListener  extends ItemListener {
    ...
    private void enableConcurrentBuild(final Item item) {
      AbstractProject<?, ?> job = (AbstractProject<?, ?>) item;
      if(!job.isConcurrentBuild()) {
        try {
          LOGGER.info("Enabling concurrent build for job: " +job.getDisplayName());
          job.setConcurrentBuild(true);
        } catch (IOException e) {
          LOGGER.warning("Exception while enabling concurrent build for job: " + job.getDisplayName() + "\n" + e);
        }
      }
    }
